...
| `storage_state` | string | `null` | Path to a saved session file (generated by `writechoice session`). Loads cookies and localStorage into the browser context. |
...
For sites behind a login, use `writechoice session` to capture a real browser session, then reference the saved file with `storage_state`. See [Authenticated Scraping](./authenticated-scraping) for the full workflow.
