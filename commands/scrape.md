...
| `storage_state` | Path to a session file from `writechoice session` | `null` |
...
For sites behind a login, use [writechoice session](../session.md) to capture your authenticated session, then reference it in `playwright_config.storage_state`.
