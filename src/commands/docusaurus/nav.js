 ...
 * @param {string} [options.linksDir] - Directory for type:"link" sidebar entry stubs (default: links)
 * @param {boolean} [options.noLinks] - Skip stubbing type:"link" entries (they're dropped, as before)
...
export async function docusaurusNav(file, options = {}) {
  ...
  const ctx = { prefix, docsDir, linksDir, noLinks: Boolean(options.noLinks) };

  const navigation = buildNavigation(sidebars, keys, ctx);
  ...
  if (verbose) {
    ...
    if (!options.noLinks && existsSync(linksDir)) {
      const stubs = readdirSync(linksDir).filter((f) => f.endsWith(".mdx"));
      if (stubs.length) {
...
function buildNavigation(sidebars, keys, ctx) {
  if (keys.length === 1) {
    const pages = convertItems(sidebars[keys[0]], ctx);
    return {
      anchors: [
        {
  ...
  const tabs = keys.map((key) => ({
    ...
    pages: convertItems(sidebars[key], ctx),
  }));
  return { tabs };
}

function convertItems(items, ctx) {
  if (!Array.isArray(items)) return [];
  return items.flatMap((item) => {
    const result = convertItem(item, ctx);
...
function convertItem(item, ctx) {
  ...
  switch (item.type) {
    case "category": {
      ...
      group.pages = convertItems(item.items || [], ctx);
      return group;
    }
    ...
    case "link":
      // { type: 'link', label: '...', href: '...' } — external or absolute-path
      // sidebar link. Stub it out as a page carrying `url` frontmatter, same
      // mechanism as `wcc readme nav`'s external link handling, so it survives
      // the conversion instead of silently disappearing from the nav.
      if (!item.href) return null;
      if (ctx.noLinks) return null;
      return writeLinkStub(ctx.linksDir, item.label || item.href, item.href);

    case "html":
      // Raw HTML separator — no destination, nothing to preserve.
      return null;
...
// ---------------------------------------------------------------------------
// Link stubs (type: "link" sidebar entries)
// ---------------------------------------------------------------------------
