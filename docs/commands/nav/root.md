...
The command walks the `docs.json` navigation tree and applies a promotion check to every group that is **nested inside another group's `pages` array**. Top-level groups — those that are direct children of `tabs`, `menu`, `anchors`, `dropdowns`, `versions`, `languages`, or `products` — are never touched.

For each eligible nested group:

1. Look at `pages[0]` — if it is not a string (it's a sub-group), skip.
2. Find the file on disk. If the path from `docs.json` doesn't exist yet (e.g. before running `nav folders`), the command falls back to searching for the filename anywhere in the project.
...
The command first tries the exact path from `docs.json`. If the file doesn't exist there (common before running `nav folders`), it searches the entire project for a file with the same basename. If multiple files share that name the check is skipped for that entry.
