...
check
  ...
  .option("-c, --concurrency <number>", "Number of parallel requests (default: 20, or 3 for --local)")
...
nav
  ...
  .option(
    "--base [dir]",
    "Keep each file's original base folder (no value), use a fixed prefix (e.g. docs), or omit for config default",
  )
  .option(
    "--skip-level <n>",
    "Skip a navigation level by number (repeatable, 1-based)",
    (v, acc) => [...acc, parseInt(v, 10)],
    [],
  )
...
docusaurus
  .command("slugify <folder>")
  .description("Rename converted MDX files to match their frontmatter slug / id")
  .option("--docs <file>", "Path to docs.json to update (default: ./docs.json)")
  .option("--dry-run", "Preview renames without writing files")
  .option("--quiet", "Suppress terminal output")
  .action(async (folder, options) => {
    const { slugifyDocusaurus } = await import("../src/commands/docusaurus/slugify.js");
    await slugifyDocusaurus(folder, options);
  });
...
docusaurus
  ...
  .option("--docs-dir <dir>", "Docs folder for autogenerated items (default: docs/ next to sidebars.js)")
  .option("-o, --output <file>", "Output file (default: nav.json)")
  .option("--links-dir <dir>", "Directory for type:\"link\" sidebar entry stubs (default: links)")
  .option("--no-links", "Skip stubbing type:\"link\" entries (they're dropped instead)")
  .option("--quiet", "Suppress terminal output")
  .action(async (file, options) => {
    ...
    options.noLinks = options.links === false;
